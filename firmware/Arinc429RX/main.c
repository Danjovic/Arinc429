...
int main(void) {
  ...
  while (1) {
    ...
    switch (tolower(KeyStroke)) {
        ...
        /* Output */
      case 'p':
        printf_P(PSTR("\n\nArinc 429 sniffer V1.21 (c)2013-2026 by Daniel Jose Viana"));
        printf_P(PSTR("\nOutput:\nV - Set Verbose Mode\nQ - Set Quiet Mode"));
...
void Print_Message(bool showDeltaTime) {
  ...
  if (i & 0x02) putc('1', &usart0_str);
  else putc('0', &usart0_str);
  if (i & 0x01) putc('1', &usart0_str);
  else putc('0', &usart0_str);
  ...
  i = p_getb->word_arinc.bytes.byte3;  // SSM = bits 5,6
  if (i & 0x40) putc('1', &usart0_str);
  else putc('0', &usart0_str);
  if (i & 0x20) putc('1', &usart0_str);
  else putc('0', &usart0_str);
  ...
  switch (config_mode & 0x03) {
    case 0:  // binario
      for (uint8_t j = 0; j < 19; j++) {
        if (databits & 0x40000) putc('1', &usart0_str);
        else putc('0', &usart0_str);
        databits <<= 1;
      }
  ...
  // print Parity
  //  i = p_getb->word_arinc.bytes.byte3;  // parity = bits 0,1
  if (__verbose) printf_P(PSTR("P:"));

  if (i & 0x80) putc('1', &usart0_str);
  else putc('0', &usart0_str);
